    ...
    return (
        <main style={{ minHeight: '100vh' }}>
            ...
            <section style={{ maxWidth: 1240, margin: '0 auto', padding: 'clamp(60px, 8vw, 96px) 32px 56px' }} className="px-4 pb-14 sm:px-6 lg:px-8">
                <div style={{ fontFamily: 'var(--font-mono), monospace', fontSize: 10, letterSpacing: '0.18em', color: 'var(--accent)', textTransform: 'uppercase', marginBottom: 16 }}>BLOG</div>
            ...
            {categories.length > 0 && (
                <div style={{ borderTop: '1px solid var(--line)', borderBottom: '1px solid var(--line)', background: 'var(--panel2)' }}>
                        <div style={{ maxWidth: 1240, margin: '0 auto', padding: '16px 32px', display: 'flex', gap: 8, flexWrap: 'wrap' }} className="px-4 sm:px-6 lg:px-8">
                        <Link href="/blog" style={{ fontFamily: 'var(--font-mono), monospace', fontSize: 12, padding: '7px 14px', borderRadius: 999, border: '1px solid var(--accent)', background: 'color-mix(in srgb, var(--accent) 15%, var(--panel))', color: 'var(--text)', textDecoration: 'none' }}>Wszystkie</Link>
                        {categories.map((cat: any) => (
                            <Link key={cat._id} href={`/blog/kategoria/${cat.slug.current}`} style={{ fontFamily: 'var(--font-mono), monospace', fontSize: 12, padding: '7px 14px', borderRadius: 999, border: '1px solid var(--line)', background: 'transparent', color: 'var(--muted)', textDecoration: 'none' }}>
                                {cat.title}
                            </Link>
                        ))}
                    </div>
            ...
            <div style={{ maxWidth: 1240, margin: '0 auto', padding: '48px 32px 88px' }} className="px-4 py-14 sm:px-6 lg:px-8">
                ...
                {mainPost && (
                    <div style={{ marginBottom: 64 }}>
                        <div style={{ fontFamily: 'var(--font-mono), monospace', fontSize: 10, letterSpacing: '.1em', color: 'var(--accent)', textTransform: 'uppercase', marginBottom: 20 }}>WYRÓŻNIONY ARTYKUŁ</div>
                        <Link href={`/blog/${mainPost.slug.current}`} className="ct-card-hover grid grid-cols-1 overflow-hidden rounded-[24px] border border-[var(--line)] bg-[var(--panel)] text-[var(--text)] no-underline lg:grid-cols-2" style={{ gap: 0, alignItems: 'stretch' }}>
                            ...
                            <div style={{ padding: '36px 36px 36px 42px', display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
                                {mainPost.categories?.slice(0, 1).map((cat: any) => (
                                    <div key={cat._id} style={{ fontFamily: 'var(--font-mono), monospace', fontSize: 10, letterSpacing: '.1em', color: 'var(--accent)', textTransform: 'uppercase', marginBottom: 14 }}>{cat.title}</div>
                                ))}
                                <h2 style={{ fontFamily: 'var(--font-space), sans-serif', fontWeight: 700, fontSize: 'clamp(20px, 2.3vw, 30px)', letterSpacing: '-0.045em', lineHeight: 1.12, margin: '0 0 14px' }}>{mainPost.title}</h2>
                ...
                {regularPosts.length > 0 && (
                    <div>
                        <div style={{ fontFamily: 'var(--font-mono), monospace', fontSize: 10, letterSpacing: '.1em', color: 'var(--accent)', textTransform: 'uppercase', marginBottom: 28 }}>WSZYSTKIE ARTYKUŁY</div>
                                ...
                                <Link key={post._id} href={`/blog/${post.slug.current}`} className="ct-card-hover" style={{ textDecoration: 'none', color: 'var(--text)', display: 'flex', flexDirection: 'column', borderRadius: 20, background: 'var(--panel)', border: '1px solid var(--line)', overflow: 'hidden' }}>
                                    ...
                                    <div style={{ padding: '18px 20px', display: 'flex', flexDirection: 'column', flex: 1 }}>
                                        {post.categories?.slice(0, 1).map((cat: any) => (
                                            <div key={cat._id} style={{ fontFamily: 'var(--font-mono), monospace', fontSize: 10, letterSpacing: '.1em', color: 'var(--accent)', textTransform: 'uppercase', marginBottom: 8 }}>{cat.title}</div>
                                        ))}
                                        <h3 style={{ fontFamily: 'var(--font-space), sans-serif', fontWeight: 600, fontSize: 17, lineHeight: 1.25, margin: '0 0 10px' }}>{post.title}</h3>
